...
**Direct-mapped**: a given memory block can be placed in only one possible location in the cache
...
**Write-back cache**: combine multiple writes to the same block before eviction, BUT need a bit in tag store indicating that block is dirty
