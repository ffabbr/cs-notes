...
export const SocialImage: SocialImageOptions["imageStructure"] = ({
  ...
  const accent = "#fed588"
  const text = "#ffffff"
  const secondaryText = "#d4d4d4"
  const useSmallerFont = title.length > 22
    ...
    >
      ...
      >
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          >
            {iconBase64 && (
              <img src={iconBase64} width={64} height={64} style={{ borderRadius: "50%" }} />
            )}
            <p
              style={{
                color: accent,
        ...
        >
          <div style={{ display: "flex", gap: "1rem" }}>
            {fileData.frontmatter?.tags?.slice(0, 4).map((tag) => (
              <span
                style={{
